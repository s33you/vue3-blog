...
<p>vue3暴露了一系列 <code>api</code>用于创建脱离模板的响应式对象,这些内容的本质是Proxy的封装</p>
...
<p>1.当 ref 作为setup()的上下文,而且作为 setup ()返回的对象的属性时，它会自动解包到内部值。没有必要附加.value</p>
...
<h4>2.计算属性和监听器</h4>
<p>vue3可以将计算属性和监听器分离，暴露为单独的api，建立一个计算属性函数或者监听器<br>
<strong>计算属性</strong></p>
...
<p>plusOne 的值依赖于<code>()=&gt;count.value + 1</code>这个getter函数，<code>computed</code>函数返回一个reactive对象，且这个对象不可变<br>
但是，一切并非这么绝对，当你传第一个拥有，<code>get</code>，<code>set</code>函数的对象时，<code>computed</code>函数将返回一个可写的响应式对象</p>
